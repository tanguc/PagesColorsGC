...
public class Monitor {
  ...
  public void lockNoHandshake() {
    RVMThread.getCurrentThread().handOverGlobaliserQueue();
    int mySlot = RVMThread.getCurrentThreadSlot();
    if (mySlot != holderSlot) {
      if (VM.VerifyAssertions) VM._assert(monitor != null);
    ...
    recCount++;
    acquireCount++;
    RVMThread.getCurrentThread().reacquireGlobaliserQueue();
  }
  @NoInstrument
  @NoInline
  @NoOptCompile
  public void lockNoHandshakeNI() {
    int mySlot = RVMThread.getCurrentThreadSlotNI();
    if (mySlot != holderSlot) {
      sysCall.sysMonitorEnter(monitor);
      if (VM.VerifyAssertions) VM._assert(holderSlot==-1);
      if (VM.VerifyAssertions) VM._assert(recCount==0);
  ...
  @NoInline
  @NoOptCompile
  public void lockNoHandshakeNIDbg() {
    int mySlot = RVMThread.getCurrentThreadSlotNI();
    if (mySlot != holderSlot) {
      if (VM.VerifyAssertions) VM._assert(monitor != null);
      sysCall.sysMonitorEnterDbg(monitor);
      if (VM.VerifyAssertions) VM._assert(holderSlot==-1);
      if (VM.VerifyAssertions) VM._assert(recCount==0);
      holderSlot = mySlot;
    }
  ...
  @NoInline
  @NoOptCompile
  public void unlockNIDbg() {
    if (--recCount==0) {
      holderSlot=-1;
      if (VM.VerifyAssertions) VM._assert(monitor != null);
      sysCall.sysMonitorExitDbg(monitor);
    }
  }
  /**
  ...
  public void waitNoHandshakeNI() {
    ...
    if (VM.VerifyAssertions) VM._assert(this.recCount==0);
    this.recCount=recCount;
    holderSlot=RVMThread.getCurrentThreadSlotNI();
  }
  @NoInstrument
  @NoInline
  @NoOptCompile
  public void waitNoHandshakeNIDbg() {
    int recCount=this.recCount;
    this.recCount=0;
    holderSlot=-1;
    if (VM.VerifyAssertions) VM._assert(monitor != null);
    sysCall.sysMonitorWaitDbg(monitor);
    if (VM.VerifyAssertions) VM._assert(holderSlot==-1);
    if (VM.VerifyAssertions) VM._assert(this.recCount==0);
    this.recCount=recCount;
    holderSlot=RVMThread.getCurrentThreadSlotNI();
  ...
  @NoOptCompile
  @NoInstrument
  public void broadcastNIDbg() {
	if (VM.VerifyAssertions) VM._assert(monitor != null);
    sysCall.sysMonitorBroadcastDbg(monitor);
  }
  /**
  ...
  @NoOptCompile
  @NoInstrument
  public void lockedBroadcastNoHandshakeNI() {
    lockNoHandshakeNI();
    broadcastNI();
    unlockNI();
  }
