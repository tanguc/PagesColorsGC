...
public final class RVMThread extends ThreadContext {
  private static final int[] balancedBinding = {0, 16, 32, 48, 8, 24, 40, 56, 1, 17, 33, 49, 9, 25, 41, 57, 2, 18, 34, 50, 10, 26, 42, 58, 3, 19, 35, 51, 11, 27, 43, 59, 4, 20, 36, 52, 12, 28, 44, 60, 5, 21, 37, 53, 13, 29, 45, 61, 6, 22, 38, 54, 14, 30, 46, 62, 7, 23, 39, 55, 15, 31, 47, 63};
  private static final int[] allOnOneNode = {0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7};
  private static final int[] allOnOneSocket = {0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15, 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15, 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15, 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15};
  private static final int[] oneProcPerNode = {0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56, 0, 8, 16, 24, 32, 40, 48, 56};
  private static final int[] allOneHop = {0, 16, 32, 48, 1, 17, 33, 49, 2, 18, 34, 50, 3, 19, 35, 51, 4, 20, 36, 52, 5, 21, 37, 53, 6, 22, 38, 54, 7, 23, 39, 55, 0, 16, 32, 48, 1, 17, 33, 49, 2, 18, 34, 50, 3, 19, 35, 51, 4, 20, 36, 52, 5, 21, 37, 53, 6, 22, 38, 54, 7, 23, 39, 55};
  private static final int[] oppositeConsecutive = {0, 56, 32, 24, 48, 40, 16, 8, 1, 57, 33, 25, 49, 41, 17, 9, 2, 58, 34, 26, 50, 42, 18, 10, 3, 59, 35, 27, 51, 43, 19, 11, 4, 60, 36, 28, 52, 44, 20, 12, 5, 61, 37, 29, 53, 45, 21, 13, 6, 62, 38, 30, 54, 46, 22, 14, 7, 63, 39, 31, 55, 47, 23, 15};
  
  private static final int[] cpuBinding = balancedBinding;
  ...
  public static class GCBlockAdapter extends BlockAdapter {
    ...
    @Override
    boolean hasBlockRequest(RVMThread t) {
      return t.shouldBlockForGC && !t.isBlockingForTIGC;
    }
  ...
  @Uninterruptible
  @NonMoving
  public static class TIGCBlockAdapter extends BlockAdapter {
    @NoInstrument
    boolean isBlocked(RVMThread t) {
      return t.isBlockedForTIGC;
    }
  
    void setBlocked(RVMThread t, boolean value) {
      t.isBlockedForTIGC = value;
    }
  
    int requestBlock(RVMThread t) {
      if (!t.isBlockedForTIGC) {
        t.shouldBlockForTIGC = true;
      }
      return 0;
    }
  
    boolean hasBlockRequest(RVMThread t) {
      return t.shouldBlockForTIGC;
    }
    ...
    void clearBlockRequest(RVMThread t) {
      t.shouldBlockForTIGC = false;
    }
  }

  public static final GCBlockAdapter gcBlockAdapter = new GCBlockAdapter();
  public static final TIGCBlockAdapter TIGCBlockAdapter = new TIGCBlockAdapter();

  static final BlockAdapter[] blockAdapters = new BlockAdapter[] {
    suspendBlockAdapter, handshakeBlockAdapter, gcBlockAdapter, TIGCBlockAdapter };
  ...
  public static RVMThread[] threadMonotonically = new RVMThread[MAX_THREADS];
  public static boolean[] benchmarkThread = new boolean[MAX_THREADS];
  ...
  public static int nextMonotonicID = 2;
  @Entrypoint
  public int monotonicID;
  
    
  public static final int CATEGORY_BOOT = 1;
  public static final int CATEGORY_TIMER = 2;
  public static final int CATEGORY_FINALISER = 3;
  public static final int CATEGORY_COLLECTOR = 4;
  public static final int CATEGORY_OPTIMISING = 5;
  public static final int CATEGORY_BENCHMARK = 6;
  public static final int CATEGORY_GLOBALISER = 7;
  ...
  @NoInstrument
  public static boolean isBenchmarkThread(int threadID) {
    return benchmarkThread[threadID];
  }
  ...
  @NoInstrument
  public NoYieldpointsMonitor monitorNI() {
    return monitorBySlot[threadSlot];
  }
  ...
  public static int currentCPUCounter = 0;
  public int boundProcessor = -1;
  ...
  void assignThreadSlot() {
    if (!VM.runningVM) {
      // primordial thread
      threadSlot = 1;
      category = RVMThread.CATEGORY_BOOT;
      monotonicID = 1;
      threadMonotonically[1] = this;
      benchmarkThread[1] = false;
      threadBySlot[1] = this;
      threads[0] = this;
      threadIdx = 0;
      numThreads = 1;
    } else {
      threadMonotonically[nextMonotonicID] = this;
      monotonicID = nextMonotonicID;
      nextMonotonicID++;
      if (name.equals("TimerThread")) {
        category = RVMThread.CATEGORY_TIMER;
        benchmarkThread[monotonicID] = false;
      }
      else if (name.equals("FinalizerThread")) {
        category = RVMThread.CATEGORY_FINALISER;
        benchmarkThread[monotonicID] = false;
      }
      else if (name.startsWith("org.mmtk.plan.marksweep.MSCollector") || name.startsWith("org.mmtk.plan.ControllerCollectorContext") || name.startsWith("org.mmtk.plan.ThreadIndependentControllerCollectorContext")) {
        category = RVMThread.CATEGORY_COLLECTOR;
        benchmarkThread[monotonicID] = false;
      }
      else if ((name.equals("Organizer") || name.equals("OSR_Organizer") || name.equals("CompilationThread") || name.equals("ControllerThread"))) {
        category = RVMThread.CATEGORY_OPTIMISING;
        benchmarkThread[monotonicID] = false;
      }
      else if (name.equals("GlobaliserThread")) {
        category = RVMThread.CATEGORY_GLOBALISER;
        benchmarkThread[monotonicID] = false;
      }
      else {
        category = RVMThread.CATEGORY_BENCHMARK;
        benchmarkThread[monotonicID] = true;
        
        int nC = VM.numberCPUs;
                 
        if (Plan.SETBIND && Options.verbose.getValue() >= 5) {
          VM.sysWrite("Binding thread ");
          VM.sysWrite(monotonicID);
          VM.sysWrite(" to available core ");
          VM.sysWrite(currentCPUCounter);
          VM.sysWrite(" of ");
          VM.sysWriteln((nC < sysCall.sysNumProcessors()) ? nC : sysCall.sysNumProcessors());
        }
        boundProcessor = cpuBinding[currentCPUCounter];
        if (Plan.SETBIND) sysCall.sysSetBind(boundProcessor);
        currentCPUCounter = (currentCPUCounter + 1) % ((nC < sysCall.sysNumProcessors()) ? nC : sysCall.sysNumProcessors());
        if (Plan.SETBIND && Options.verbose.getValue() >= 2) {
           VM.sysWrite("Assigned core for thread ");
          VM.sysWrite(monotonicID);
          VM.sysWrite(": ");
          VM.sysWriteln(sysCall.sysGetBind());
        }
      }
  ...
  void releaseThreadSlot() {
   ...
   public RVMThread(byte[] stack, Thread thread, String name, boolean daemon, SystemThread systemThread, int priority) {
    ...
    //In classpath, Thread's have an associated VMThread. This is created when we do thread.start()
    // This code triggers VMThread.create which in turn results here. Therefore the thread has been created and is ABOUT to start.
        
    if (Properties.fullyFullyBooted) {
      ObjectReference t = ObjectReference.fromObject(thread);
      if (MiscHeader.getColour(t) <= MiscHeader.WHITE) {
        MiscHeader.setColour(t, MiscHeader.GREY);
        MutatorContext owner = RVMThread.threadMonotonically[MiscHeader.getOriginalAllocatingThread(ObjectReference.fromObject(t))];
        if (owner != null) {
           owner.addIntoRemset(ObjectReference.fromObject(t));
         }
       }
       RVMType type = Magic.getObjectType(t);
       int[] offsets = type.getReferenceOffsets();
       if (offsets != null) {
         for (int i = 0; i < offsets.length; i++) {
           ObjectReference child = t.toAddress().plus(offsets[i]).loadObjectReference();
           if (child != null) {
             if (MiscHeader.getColour(child) <= MiscHeader.WHITE) {
               MiscHeader.setColour(child, MiscHeader.GREY);
               MutatorContext owner = RVMThread.threadMonotonically[MiscHeader.getOriginalAllocatingThread(ObjectReference.fromObject(child))];
               if (owner != null) {
                 owner.addIntoRemset(ObjectReference.fromObject(child));
               }
             }
           }
         }
    ...
    if (!VM.runningVM) {
      ...
      initMutator(threadSlot, monotonicID);
      ...
      if (Options.verbose.getValue() >= 4) {
        RVMThread.logStr("THREAD slot=");
        RVMThread.logInt(threadSlot);
        RVMThread.logStr(" ");
        RVMThread.logStr(name);
        RVMThread.logStr(" mid=");
        RVMThread.logInt(monotonicID);
        RVMThread.logStrln("");
      }
      
      //logThread
      char[] buf = java.lang.JikesRVMSupport.getBackingCharArrayNI(name);
      int len = java.lang.JikesRVMSupport.getStringLengthNI(name);
      sysCall.logThread(monotonicID, monotonicID, category, len);
      for (int i = 0; i < len; i++) {
        sysCall.logExtraChar(monotonicID, buf[i]);
      }
      if (len % 4 != 0) {
        for (int j = 0; j < (4 - (len % 4)); j++) {
          sysCall.logExtraChar(monotonicID, (char) 255);
        }
      }
  ...
  private void checkBlockNoSaveContext() {
    ...
    reacquireGlobaliserQueue();

    if (traceBlock)
      VM.sysWriteln("Thread #", threadSlot, " is unblocked");
  }
  
  public int nestedHandovers = 0;
  public boolean terminating = false;
  
  @NoInstrument
  public void handOverGlobaliserQueue() {
    if (Plan.GLOBALISE_ON) {
      nestedHandovers++;
       
      if (nestedHandovers == 1 && !terminating) {
         if (RVMThread.getCurrentThreadMonotonicNI() != GlobaliserThread.threadID && !this.isCollectorThread()) {
           GlobaliserThread.surrenderGlobaliserQueueControl();
         }
      }
    }
  }
  ...
  @NoInstrument
  public void reacquireGlobaliserQueue() {
    if (Plan.GLOBALISE_ON) {
      if (nestedHandovers == 1 && !terminating) {
        if (RVMThread.getCurrentThreadMonotonicNI() != GlobaliserThread.threadID && !this.isCollectorThread()) {
          GlobaliserThread.reacquireGlobaliserQueueControl();
        }
      }
      nestedHandovers--;
    }
        ...
        if (newState == IN_JAVA_TO_BLOCK) {
          if (!asynchronous) {
            ...
            while (ba.hasBlockRequest(this, token) && !ba.isBlocked(this) && !isAboutToTerminate) {
              ...
              if (Plan.GLOBALISE_ON) {
                GlobaliserThread.processThread(RVMThread.getCurrentThreadMonotonicNI(), true);
              }
            }
            if (isAboutToTerminate) {
              result = TERMINATED;
  ...
  @NoInstrument
  public void enableYieldpointsNI() {
    ++yieldpointsEnabledCount;
    if (VM.VerifyAssertions)
      VM._assert(yieldpointsEnabledCount <= 1);
    if (yieldpointsEnabledCount == 1 && yieldpointRequestPending) {
      takeYieldpoint = 1;
      yieldpointRequestPending = false;
    }
  }
  ...
  @NoInstrument
  public void disableYieldpointsNI() {
    --yieldpointsEnabledCount;
  }
  ...
  @NoInstrument
  public static int getCurrentThreadSlotNI() {
    return Magic.getESIAsThread().threadSlot;
  }
    
  @NoInstrument
  public static int getCurrentThreadMonotonicNI() {
    return Magic.getESIAsThread().monotonicID;
  }
  ...
  public void terminate() {
    ...
    int threadID = RVMThread.getCurrentThreadMonotonicNI();
    for (int cId = 0; cId < MutatorContext.NUM_COUNTERS; cId++) {
      sysCall.logThreadCounter(threadID, threadID, cId, RVMThread.getCurrentThread().counters[cId]);
    }
  ...
  @Inline
  @NoInstrument
  private Address getStackTrampolineBridgeIPNI() { return Magic.objectAsAddress(stackTrampolineBridgeInstructions); }
  ...
  @Inline
  @NoInstrument
  public Address getTrampolineHijackedReturnAddressNI() { return hijackedReturnAddress; }
  ...
  @Inline
  @NoInstrument
  public static boolean isTrampolineIPNI(Address ip) { return getCurrentThread().getStackTrampolineBridgeIPNI().EQ(ip); }
  ...
  public static void blockAllMutatorsForGC() {
    boolean observedAThreadBlockedOutsideGC = false;
    //Set this to true if we have observed at least one thread being blocked for TIGC
    //This ensures we block all threads and do not break out of the loop on Step 3.
    //This method is wrapped in a while (true) infinite loop which is only terminated if
    //  we have no handshake threads left to block AND we have observed no threads in minor collection
    //If we observe threads in minor collection, we block all threads that we can and go round the loop
    //again, until all minor collections have finished and we can block those threads as well.
    while (true) {
      RVMThread.handshakeLock.lockNoHandshake(); //Thanks to the additional of TIGC, there is a chance for deadlock
                                                 // if we don't release the handshake lock periodically to allow TIGC
                                                 // to mark threads as unblocked when TIGC finishes.
      observedAThreadBlockedOutsideGC = false;
      ...
      for (int i = 0; i < RVMThread.numThreads; i++) {
        ...
        if (!t.isCollectorThread() && !t.ignoreHandshakesAndGC() && !(GlobaliserThread.threadID == t.monotonicID)) {
      ...
      // Skip over threads in TIGC. We will block them later once they have finished TIGC,
      //   in another iteration of this infinite loop.
      for (int i = 0; i < numToHandshake; i++) {
        RVMThread t = RVMThread.handshakeThreads[i];
        t.monitor().lockNoHandshake();
        if (t.blockedFor(TIGCBlockAdapter) || t.isBlockingForTIGC) {
          observedAThreadBlockedOutsideGC = true;
        }
        else if (t.blockedFor(RVMThread.gcBlockAdapter) || RVMThread.notRunning(t.asyncBlock(RVMThread.gcBlockAdapter))) {
      ...
      //     terminating).
      // WE MUST ONLY EXIT THIS INFINITE LOOP IF THERE ARE NO MORE THREADS TO BLOCK
      // __AND__ THERE ARE NO THREADS IN TIGC
      // Full heap collection is thus stalled until all TIGCs complete.
      if (numToHandshake == 0 && !observedAThreadBlockedOutsideGC) {
      ...
      RVMThread.handshakeLock.unlock();
      RVMThread.handshakeLock.broadcast();
    }
  ...
  @NoCheckStore
  @Unpreemptible
  public static void waitUntilMutatorBlockedForTIGC(int monotonicID) {
    while (true) {
      RVMThread t = RVMThread.threadMonotonically[monotonicID];
      if (VM.VerifyAssertions) VM._assert(!t.isCollectorThread() && !t.ignoreHandshakesAndGC());

      t.monitor().lockNoHandshake();
      if (t.blockedFor(RVMThread.TIGCBlockAdapter)) {
        t.monitor().unlock();
        break;
      }
      t.monitor().unlock();
  ...
  @NoCheckStore
  @Unpreemptible
  public static void blockMutatorForTIGC(int monotonicID) {
    while (true) {
      ...
      t.monitor().lockNoHandshake();
      if (t.blockedFor(RVMThread.TIGCBlockAdapter) || RVMThread.notRunning(t.asyncBlock(RVMThread.TIGCBlockAdapter))) {
        t.monitor().unlock();
        RVMThread.handshakeLock.unlock();
        break;
      }
      ...
      //Thread not blocked yet as we would like. Request block and let a new iteration begin.
      t.block(RVMThread.TIGCBlockAdapter);
      RVMThread.handshakeLock.unlock();
    }
  }
  ...
  @NoCheckStore
  @Unpreemptible
  public static void blockMutatorForGC(int monotonicID) {
    while (true) {
      RVMThread.handshakeLock.lockNoHandshake();
      ...
      RVMThread t = RVMThread.threadMonotonically[monotonicID];
      if (VM.VerifyAssertions) {
        VM._assert(!t.isCollectorThread() && !t.ignoreHandshakesAndGC());
      }
      RVMThread.acctLock.unlock();
  ...
  @NoCheckStore
  @Unpreemptible
  public static void unblockMutatorForTIGC(int monotonicID) {
    RVMThread t = RVMThread.threadMonotonically[monotonicID];
    if (VM.VerifyAssertions) VM._assert(!t.isCollectorThread() && !t.ignoreHandshakesAndGC());
  
    RVMThread.TIGCBlockAdapter.clearBlockRequest(t);
    RVMThread.TIGCBlockAdapter.setBlocked(t, false);
    t.monitor().broadcast();
  }
  
  @NoCheckStore
  @Unpreemptible
  public static void unblockMutatorForGC(int monotonicID) {
    RVMThread.handshakeLock.lockNoHandshake();
    ...
    t.unblock(RVMThread.gcBlockAdapter);
...
*  */
  ...
  public void join(long ms, int ns) throws InterruptedException {
    ...
    //RVMThread.threadMonotonically[getCurrentThreadMonotonicNI()].handOverGlobaliserQueue();
    synchronized (this) {
      if (ms == 0 && ns == 0) {
        while (!isJoinable) {
          wait(this);
    ...
    //RVMThread.threadMonotonically[getCurrentThreadMonotonicNI()].reacquireGlobaliserQueue();
  }
  ...
  */
  ...
  static void tracebackWithoutLock() {
    if (VM.runningVM) {
      RVMThread.logStr("Thread #");
      RVMThread.logIntln(getCurrentThreadSlot());
      dumpStackNI(Magic.getCallerFramePointer(Magic.getFramePointer()));
    } else {
      dumpStackNI();
    }
  }
  ...
  @UninterruptibleNoWarn("Never blocks")
  @NoInstrument
  public static void dumpStackNI() {
    if (VM.runningVM) {
      logStr("Dumping stack for Thread #");
      sysCall.logInt(getCurrentThreadSlotNI());
      logStr(" (Monotonic: ");
      sysCall.logInt(getCurrentThreadMonotonicNI());
      logStrln(")");
      Address fp = Magic.getFramePointer();
  ...
  @NoInstrument
  public static void dumpStackNI(Address fp) {
    if (VM.VerifyAssertions) {
      VM._assert(VM.runningVM);
    }
    Address ip = Magic.getReturnAddress(fp);
  ...
  @NoInstrument
  public static void dumpStackNI(Address ip, Address fp) {
    logStrln("-- STACK --");
    while (Magic.getCallerFramePointer(fp).NE(StackframeLayoutConstants.STACKFRAME_SENTINEL_FP)) {
      if (!MemoryManager.addressInVMNI(ip)) { // if code is outside of RVM heap, assume it to be native code, skip to next frame
        showMethodNI("native frame", fp);
        ip = Magic.getReturnAddress(fp);
        fp = Magic.getCallerFramePointer(fp);
      } else {
        int compiledMethodId = Magic.getCompiledMethodID(fp);
        logStr("FP="); logHex(fp); logStr(" METHOD=");
        if (compiledMethodId == StackframeLayoutConstants.INVISIBLE_METHOD_ID) {
          showMethodNI("invisible method", fp);
        } else {
          // normal java frame(s)
          CompiledMethod compiledMethod = CompiledMethods.getCompiledMethodNI(compiledMethodId);
          if (compiledMethod == null) {
            showMethodNI(compiledMethodId, fp);
          } else if (compiledMethod.getCompilerType() == CompiledMethod.TRAP) {
            showMethodNI("hardware trap", fp);
          } else {
            RVMMethod method = compiledMethod.getMethodNI();
            Offset instructionOffset = compiledMethod.getInstructionOffsetNI(ip); //Up to here
            int lineNumber = compiledMethod.findLineNumberForInstructionNI(instructionOffset);
            boolean frameShown = false;
            if (VM.BuildForOptCompiler && compiledMethod.getCompilerTypeNI() == CompiledMethod.OPT) {
              OptCompiledMethod optInfo = (OptCompiledMethod) compiledMethod;
              // Opt stack frames may contain multiple inlined methods.
              OptMachineCodeMap map = optInfo.getMCMapNI();
              int iei = map.getInlineEncodingForMCOffsetNI(instructionOffset);
              if (iei >= 0) {
                int[] inlineEncoding = map.inlineEncoding;
                int bci = map.getBytecodeIndexForMCOffsetNI(instructionOffset);
                for (; iei >= 0; iei = OptEncodedCallSiteTree.getParentNI(iei, inlineEncoding)) {
                  int mid = OptEncodedCallSiteTree.getMethodIDNI(iei, inlineEncoding);
                  method = MemberReference.getMemberRefNI(mid).asMethodReferenceNI().getResolvedMemberNI();
                  lineNumber = ((NormalMethod) method).getLineNumberForBCIndexNI(bci);
                  showMethodNI(method, lineNumber, fp);
                  if (iei > 0) {
                    bci = OptEncodedCallSiteTree.getByteCodeOffsetNI(iei, inlineEncoding);
                  }
                }
                frameShown = true;
              }
  ...
  @NoInstrument
  private static void showPrologueNI(Address fp) {
    logStr("   at [");
    logHex(fp);
    logStr(", ");
    logHex(Magic.getReturnAddress(fp));
    logStr("] ");
  }
  ...
  @NoInstrument
  private static void showMethodNI(int compiledMethodId, Address fp) {
    showPrologueNI(fp);
    logStr("<unprintable normal Java frame: CompiledMethods.getCompiledMethod(");
    sysCall.logInt(compiledMethodId);
    logStrln(") returned null>");
  }
  ...
  private static void showMethodNI(String name, Address fp) {
    showPrologueNI(fp);
    logStr("<");
    logStr(name);
    logStrln(">");
  ...
  @NoInstrument
  private static void showMethodNI(RVMMethod method, int lineNumber, Address fp) {
    showPrologueNI(fp);
    if (method == null) {
      RVMThread.logStr("<unknown method>");
    } else {
      RVMThread.logAtom(method.getDeclaringClassNI().getDescriptorNI());
      RVMThread.logStr(" ");
      RVMThread.logAtom(method.getNameNI());
      RVMThread.logAtom(method.getDescriptorNI());
    }
    if (lineNumber > 0) {
      RVMThread.logStr(" LINE=");
      sysCall.logInt(lineNumber);
    }
    RVMThread.logStrln("");
  }
  ...
  /**
   * Dump a stack in a way that will not recurse barriers. This means some shortcuts may be taken:
   * 
   * dumpLock will not be acquired.
   * We won't monitor how many times this is called recursively
   * We assume the frame pointer is valid
   * 
   * 
   */
  @NoInstrument
  public static void logStr(String v) {
  ...
  @NoInstrument
  public static void logAtom(Atom v) {
    byte[] val = v.getValNI();
    for (int i = 0, n = val.length; i < n; ++i) {
      sysCall.logChar((char) val[i]);
    }
  }
  @NoInstrument
  public static void logStrln(String v) {
    char[] buf = java.lang.JikesRVMSupport.getBackingCharArrayNI(v);
    int len = java.lang.JikesRVMSupport.getStringLengthNI(v);
    if (len == 0) {
      sysCall.logln();
  ...
  public static void logHexln(Address addr) {
    sysCall.logHexln(addr.toInt());
  }
  public static void logInt(int v) {
    sysCall.logInt(v);
  }
  public static void logIntln(int v) {
    sysCall.logIntln(v);
  }
  public static void logHex(Address v) {
