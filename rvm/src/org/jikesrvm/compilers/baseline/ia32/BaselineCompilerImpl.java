...
public abstract class BaselineCompilerImpl extends BaselineCompiler implements BaselineConstants {
  ...
  protected final void emit_aaload() {
    ...
    if (NEEDS_OBJECT_ALOAD_BARRIER && !method.hasNoInstrumentAnnotation()) {
  ...
  protected final void emit_laload() {
    ...
    if (VM.BuildFor32Addr) {
      if (SSE2_BASE) {
        asm.emitMOVQ_Reg_RegIdx(XMM0, T1, T0, LONG, NO_SLOT);
  ...
  protected final void emit_aastore() {
    Barriers.compileModifyCheck(asm, 8);
    asm.emitPUSH_Imm(!method.hasNoInstrumentAnnotation() ? 0 : 1);
    if (doesCheckStore) {
      genParameterRegisterLoad(asm, 4);
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.aastoreMethodNI.getOffset()));
    } else {
      genParameterRegisterLoad(asm, 4);
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.aastoreUninterruptibleMethodNI.getOffset()));
    }
  }
  ...
  protected final void emit_unresolved_getstatic(FieldReference fieldRef) {
    ...
    if (fieldRef.getSize() <= BYTES_IN_INT) {
      ...
      if (VM.BuildFor32Addr) {
        asm.emitPUSH_RegOff(T0, BYTE, Magic.getTocPointer().toWord().toOffset().plus(WORDSIZE)); // get high part
        asm.emitPUSH_RegOff(T0, BYTE, Magic.getTocPointer().toWord().toOffset());                // get low part
      } else {
  ...
  protected final void emit_unresolved_putstatic(FieldReference fieldRef) {
    emitDynamicLinkingSequence(asm, T0, fieldRef, true);
    if (NEEDS_OBJECT_PUTSTATIC_BARRIER && fieldRef.getFieldContentsType().isReferenceType() && !method.hasNoInstrumentAnnotation()) {
      Barriers.compilePutstaticBarrier(asm, T0, fieldRef.getId());
    } else {
      if (fieldRef.getSize() <= BYTES_IN_INT) { // field is one word
        if (VM.BuildFor32Addr) {
          asm.emitPOP_RegOff(T0, BYTE, Magic.getTocPointer().toWord().toOffset());
        } else {
          asm.emitPOP_Reg(T1);
          asm.emitMOV_RegOff_Reg(T0, BYTE, Magic.getTocPointer().toWord().toOffset(), T1);
        }
      } else { // field is two words (double or long)
        if (VM.VerifyAssertions) VM._assert(fieldRef.getSize() == BYTES_IN_LONG);
        if (VM.BuildFor32Addr) {
          asm.emitPOP_RegOff(T0, BYTE, Magic.getTocPointer().toWord().toOffset());                // store low part
          asm.emitPOP_RegOff(T0, BYTE, Magic.getTocPointer().toWord().toOffset().plus(WORDSIZE)); // store high part
        } else {
          asm.emitPOP_RegOff(T0, BYTE, Magic.getTocPointer().toWord().toOffset());
          if (fieldRef.getNumberOfStackSlots() != 1) {
            adjustStack(WORDSIZE, true);
  ...
  protected final void emit_resolved_putstatic(FieldReference fieldRef) {
    RVMField field = fieldRef.peekResolvedField();
    Offset fieldOffset = field.getOffset();
    if (NEEDS_OBJECT_PUTSTATIC_BARRIER && field.isReferenceType() && !field.isUntraced() && !method.hasNoInstrumentAnnotation()) {
  ...
  protected final void emit_unresolved_getfield(FieldReference fieldRef) {
    ...
    if (fieldType.isReferenceType()) {
      ...
      if (NEEDS_OBJECT_GETFIELD_BARRIER && !method.hasNoInstrumentAnnotation()) {
  ...
  protected final void emit_resolved_getfield(FieldReference fieldRef) {
    ...
    if (field.isReferenceType()) {
      // 32/64bit reference load
      if (NEEDS_OBJECT_GETFIELD_BARRIER && !field.isUntraced() && !method.hasNoInstrumentAnnotation()) {
  ...
  protected final void emit_unresolved_putfield(FieldReference fieldRef) {
    ...
    if (fieldType.isReferenceType()) {
      ...
      if (NEEDS_OBJECT_PUTFIELD_BARRIER && !method.hasNoInstrumentAnnotation()) {
  ...
  protected final void emit_resolved_putfield(FieldReference fieldRef) {
    ...
    if (field.isReferenceType()) {
      // 32/64bit reference store
      if (NEEDS_OBJECT_PUTFIELD_BARRIER && !field.isUntraced() && !method.hasNoInstrumentAnnotation()) {
  ...
  @Override
  protected final void emit_resolved_new(RVMClass typeRef, int advice) {
    ...
    asm.emitPUSH_Imm(advice);
    genParameterRegisterLoad(asm, 8);                        // pass 7 parameter words
    asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.resolvedNewScalarMethod.getOffset()));
    asm.emitPUSH_Reg(T0);
  }

  @Override
  protected final void emit_unresolved_new(TypeReference typeRef, int advice) {
    int site = MemoryManager.getAllocationSite(true);
    asm.emitPUSH_Imm(typeRef.getId());
    asm.emitPUSH_Imm(site);            // site
    asm.emitPUSH_Imm(advice);
    genParameterRegisterLoad(asm, 3);  // pass 2 parameter words
    asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.unresolvedNewScalarMethod.getOffset()));
    asm.emitPUSH_Reg(T0);
  }

  @Override
  protected final void emit_resolved_newarray(RVMArray array, int advice) {
    ...
    asm.emitPUSH_Imm(site);
    asm.emitPUSH_Imm(advice);
    genParameterRegisterLoad(asm, 9);        // pass 8 parameter words
    asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.resolvedNewArrayMethod.getOffset()));
    asm.emitPUSH_Reg(T0);
  }

  @Override
  protected final void emit_unresolved_newarray(TypeReference tRef, int advice) {
    int site = MemoryManager.getAllocationSite(true);
    // count is already on stack- nothing required
    asm.emitPUSH_Imm(tRef.getId());
    asm.emitPUSH_Imm(site);           // site
    asm.emitPUSH_Imm(advice);
    genParameterRegisterLoad(asm, 4); // pass 3 parameter words
  ...
  private void incEdgeCounter(GPR scratch, GPR idx, int counterIdx) {
    ...
    ForwardReference fr1 = asm.forwardJcc(S);
    if (idx == null) {
