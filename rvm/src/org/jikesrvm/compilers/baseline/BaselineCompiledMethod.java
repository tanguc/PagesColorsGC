...
public final class BaselineCompiledMethod extends CompiledMethod implements BaselineConstants {
  ...
  public int findLineNumberForInstructionNI(Offset instructionOffset) {
    int bci = findBytecodeIndexForInstructionNI(instructionOffset);
    if (bci == -1) return 0;
    return ((NormalMethod) method).getLineNumberForBCIndexNI(bci);
  }
  ...
  @Uninterruptible
  @NoInstrument
  public int findBytecodeIndexForInstructionNI(Offset instructionOffset) {
    Offset instructionIndex = instructionOffset.toWord().rsha(LG_INSTRUCTION_WIDTH).toOffset();
    int candidateIndex = -1;
    int bcIndex = 0;
    Offset instrIndex = Offset.zero();
