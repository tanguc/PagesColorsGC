...
import org.jikesrvm.mm.mmtk.AllocationSite;
...
public abstract class TemplateCompilerFramework
      ...
      switch (code) {
        ...
        case JBC_new: {
          ...
          if (type != null && (type.isInitialized() || type.isInBootImage())) {
            emit_resolved_new(type.asClass(), AllocationSite.getAdvice(type.asClassNI(), method.getNameNI(), method.getDescriptorNI(), bcodes.index()));
          } else {
            if (VM.VerifyUnint && isUnpreemptible) forbiddenBytecode("unresolved new ", typeRef, bcodes.index());
            emit_unresolved_new(typeRef, AllocationSite.getAdvice(typeRef, method.getNameNI(), method.getDescriptorNI(), bcodes.index()));
          }
          break;
        }
        ...
        case JBC_newarray: {
          ...
          emit_resolved_newarray(array, AllocationSite.getAdvice(array.getTypeRef(), method.getNameNI(), method.getDescriptorNI(), bcodes.index()));
          break;
        }
        ...
        case JBC_anewarray: {
          ...
          if (array != null && (array.isInitialized() || array.isInBootImage())) {
            ...
            emit_unresolved_newarray(arrayRef, AllocationSite.getAdvice(arrayRef, method.getNameNI(), method.getDescriptorNI(), bcodes.index()));
          }
          break;
        }
  ...
  */
   ...
   */
  protected abstract void emit_resolved_new(RVMClass typeRef, int advice);
   ...
   */
  protected abstract void emit_unresolved_new(TypeReference typeRef, int advice);

  /**
   * Emit code to allocate an array
   * @param array the {@link RVMArray} to instantiate
   */
  protected abstract void emit_resolved_newarray(RVMArray array, int advice);
   ...
   */
  protected abstract void emit_unresolved_newarray(TypeReference typeRef, int advice);
