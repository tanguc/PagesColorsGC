...
public abstract class OptEncodedCallSiteTree {
  ...
  @NoInstrument
  public static int getMethodIDNI(int entryOffset, int[] encoding) {
    return encoding[entryOffset + 1];
  }
  ...
  @NoInstrument
  public static int getByteCodeOffsetNI(int entryOffset, int[] encoding) {
    return encoding[entryOffset];
  }
  ...
  @NoInstrument
  public static int getParentNI(int index, int[] encodedTree) {
    while (index >= 0 && encodedTree[index] >= -1) {
      index--;
    }
    if (index < 0) {
