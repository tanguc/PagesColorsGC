...
public abstract class CompiledMethod {
  ...
  @Uninterruptible
  @NoInstrument
  public final RVMMethod getMethodNI() {
    return method;
  }
  ...
  @Uninterruptible
  @NoInstrument
  public final Offset getInstructionOffsetNI(Address ip) {
    if (getCompilerType() == JNI || getCompilerType() == TRAP) {
      return Offset.zero();
    } else {
      Offset offset = ip.diff(Magic.objectAsAddress(instructions));
      int max = (instructions.length() + 1) << ArchitectureSpecific.ArchConstants.LG_INSTRUCTION_WIDTH;
      if (!offset.toWord().LT(Word.fromIntZeroExtend(max))) {
        if (RVMThread.isTrampolineIPNI(ip)) {
          ip = RVMThread.getCurrentThread().getTrampolineHijackedReturnAddressNI();
          offset = ip.diff(Magic.objectAsAddress(instructions));
          if (offset.toWord().LT(Word.fromIntZeroExtend(max)))
            return offset;
        }
        ...
        RVMThread.logStr("\nIn thread ");
        RVMThread.logInt(RVMThread.getCurrentThreadSlot());
        RVMThread.logStr(" getInstructionOffset: ip is not within compiled code for method: ");
        RVMThread.logHex(ip);
        RVMThread.logStrln("");
        
        RVMThread.logStr("\tsupposed method is ");
        RVMThread.logAtom(method.getMemberRefNI().getTypeNI().getNameNI());
        RVMThread.logStr(".");
        RVMThread.logAtom(method.getMemberRefNI().getNameNI());
        RVMThread.logStr(" ");
        RVMThread.logAtom(method.getMemberRefNI().getDescriptorNI());
        RVMThread.logStrln("");
        
        RVMThread.logStr("\tcode for this method starts at ");
        RVMThread.logHex(instructionStart);
        RVMThread.logStrln("");
        
        RVMThread.logStr("\t and has last valid return address of ");
        RVMThread.logHex(instructionStart.plus(max));
        RVMThread.logStrln("");
        
        RVMThread.logStr("The requested instruction address was ");
        RVMThread.logHex(ip);
        RVMThread.logStrln("");
        
        //TODO: CONVERT THE BELOW TO BE IN @NOINSTRUMENT FORM. I GOT LAZY AND DECIDED IT WASN'T COMPLETELY NECESSARY
        /*CompiledMethod realCM = CompiledMethods.findMethodForInstruction(ip);
        if (realCM == null) {
          VM.sysWriteln("\tUnable to find compiled method corresponding to this return address");
        } else {
        ...
        VM.sysWriteln("Attempting to dump virtual machine state before exiting");
        RVMThread.dumpVirtualMachine();*/
        VM.sysFail("Terminating VM due to invalid request for instruction offset whilst dumping the stack (in @NoInstrument mode)");
      }
      // NOTE: we are absolutely positive that offset will fit in 32 bits
      // because we don't create CodeArrays that are so massive it won't.
      // Thus, we do the assertion checking above to ensure that ip is in range.
  ...
  @Uninterruptible
  @NoInstrument
  public abstract int getCompilerTypeNI();
  ...
  @Uninterruptible
  @NoInstrument
  public int findLineNumberForInstructionNI(Offset instructionOffset) {
    return 0;
  }
