...
public class RuntimeEntrypoints implements ArchitectureSpecific.StackframeLayoutConstants {
   ...
   */
  @Entrypoint
  static void aastoreNI(Object[] arrayRef, int index, Object value, int noInstrumentPresent) throws ArrayStoreException, ArrayIndexOutOfBoundsException {
    checkstore(arrayRef, value);
    int nelts = ObjectModel.getArrayLength(arrayRef);
    if (index >=0 && index < nelts) {
      Services.setArrayUninterruptible(arrayRef, index, value, noInstrumentPresent);
    } else {
      throw new ArrayIndexOutOfBoundsException(index);
    }
  }
  ...
  @Entrypoint
  @Uninterruptible
  static void aastoreUninterruptibleNI(Object[] arrayRef, int index, Object value, int noInstrumentPresent) {
    if (VM.VerifyAssertions) {
      int nelts = ObjectModel.getArrayLength(arrayRef);
      VM._assert(index >=0 && index < nelts);
    }
   ...
   */
  @Entrypoint
  static Object unresolvedNewScalar(int id, int site, int advice) throws NoClassDefFoundError, OutOfMemoryError {
    TypeReference tRef = TypeReference.getTypeRef(id);
  ...
  public static Object resolvedNewScalar(int size, TIB tib, boolean hasFinalizer, int allocator, int align,
    ...
    return resolvedNewScalar(size, tib, hasFinalizer, allocator, align, offset, site, Plan.DEFAULT_ADVICE);
  }
  @Entrypoint
  public static Object resolvedNewScalar(int size, TIB tib, boolean hasFinalizer, int allocator, int align,
                                         int offset, int site, int advice) throws OutOfMemoryError {

    // GC stress testing
    if (VM.ForceFrequentGC) checkAllocationCountDownToGC();

    // Allocate the object and initialize its header
    Object newObj = MemoryManager.allocateScalar(size, tib, allocator, align, offset, site, advice);
   ...
   */
  @Entrypoint
  public static Object unresolvedNewArray(int numElements, int id, int site, int advice)
  ...
  public static Object resolvedNewArray(int numElements, int logElementSize, int headerSize, TIB tib,
                                        int allocator, int align, int offset, int site, int advice)
      throws OutOfMemoryError, NegativeArraySizeException {
    ...
    return MemoryManager.allocateArray(numElements, logElementSize, headerSize, tib, allocator, align, offset, site, advice);
  }
