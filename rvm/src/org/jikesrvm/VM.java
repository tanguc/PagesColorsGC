...
public class VM extends Properties {
  ...
  private static void finishBooting() {
    MiscHeader.bootThreadAddress = ObjectReference.fromObject(Magic.getESIAsThread()).toAddress().toInt();
    ...
    Plan.handleAllocationAdvice();
    Plan.handleUserCollectionRequest();
    MS.registerNumberVMThreads(RVMThread.nextMonotonicID);

    // Schedule "main" thread for execution.
    if (verboseBoot >= 2) VM.sysWriteln("Creating main thread");
    VM.fullyFullyBooted = true;
    // Create main thread.
  ...
  public static void sysFail(String message) {
    ...
    //bugReportMessage();
    sysCall.sysExit(EXIT_STATUS_SYSFAIL);
    if (VM.runningVM) {
  ...
  public static void sysExit(int value) {
    ...
    for (int i = 0; i < Space.spaceCount; i++) {
      Space s = Space.spaces[i];
      if (s == null) {
        continue;
      }
      ...
      sysCall.logSpace(RVMThread.getCurrentThreadMonotonicNI(), i, len);
            
      for (int j = 0; j < len; j++) {
        sysCall.logExtraChar(RVMThread.getCurrentThreadMonotonicNI(), buf[j]);
      }
      if (len % 4 != 0) {
        for (int j = 0; j < (4 - (len % 4)); j++) {
          sysCall.logExtraChar(RVMThread.getCurrentThreadMonotonicNI(), (char) 255);
        }
      }
    }
          
    for (int i = 1; i < RVMType.numTypes()+1; i++) {
      RVMType type = RVMType.getType(i);
      String name = type.getDescriptor().toString();
            
      char[] buf = java.lang.JikesRVMSupport.getBackingCharArrayNI(name);
      int len = java.lang.JikesRVMSupport.getStringLengthNI(name);
            
      type.resolve();
      RVMField[] instance = type.getInstanceFields();
      RVMField[] statics = type.getStaticFields();
      int nPrimFields = 0;
      int nRefFields = 0;
      int nStaticPrimFields = 0;
      int nStaticRefFields = 0;
      if (instance != null) {
        for (int j = 0; j < instance.length; j++) {
          if (instance[j].isReferenceType()) {
            nRefFields++;
          }
          else {
            nPrimFields++;
      ...
      sysCall.logType(RVMThread.getCurrentThreadMonotonicNI(), i, type.isArrayType(), type.isPrimitiveType(), type.hasFinalizer(), nPrimFields, nRefFields, nStaticPrimFields, nStaticRefFields, len);
      for (int j = 0; j < len; j++) {
        sysCall.logExtraChar(RVMThread.getCurrentThreadMonotonicNI(), buf[j]);
      }
      if (len % 4 != 0) {
    ...
    if (!Plan.PERFORMANCE_RUN) {
      for (int i = 0; i < RVMThread.nextMonotonicID; i++) {
        RVMThread thread = RVMThread.threadMonotonically[i];
        if (thread == null || thread.terminating) continue;
        for (int cId = 0; cId < MutatorContext.NUM_COUNTERS; cId++) {
          sysCall.logThreadCounter(i, i, cId, thread.counters[cId]);
        }
      }
    }
  ...
  public static void disableYieldpointsNI() { RVMThread.getCurrentThread().disableYieldpointsNI(); }
  public static void enableYieldpoints() { RVMThread.getCurrentThread().enableYieldpoints(); }
  public static void enableYieldpointsNI() { RVMThread.getCurrentThread().enableYieldpointsNI(); }
