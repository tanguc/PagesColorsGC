...
abstract class AbstractHashMapRVM<K, V> {
  ...
  public final V get(K key) {
    ...
    ObjectReference o = ObjectReference.fromObject(key);
    if (VM.fullyFullyBooted && !o.isNull() && MiscHeader.getColour(o) <= MiscHeader.WHITE) {
      MiscHeader.setColour(o, MiscHeader.GREY);
      MutatorContext owner = RVMThread.threadMonotonically[MiscHeader.getOriginalAllocatingThread(o)];
      if (owner != null) {
        owner.addIntoRemset(o);
      }
      else {
        RVMThread.logStrln("[Warning]: Shading object with no thread context");
      }
    }
    
    while (cur != null && !same(cur.getKey(), key)) {
      o = ObjectReference.fromObject(cur);
      if (VM.fullyFullyBooted && !o.isNull() && MiscHeader.getColour(o) <= MiscHeader.WHITE) {
        MiscHeader.setColour(o, MiscHeader.GREY);
        MutatorContext owner = RVMThread.threadMonotonically[MiscHeader.getOriginalAllocatingThread(o)];
        if (owner != null) {
      ...
      o = ObjectReference.fromObject(cur.getKey());
      if (VM.fullyFullyBooted && !o.isNull() && MiscHeader.getColour(o) <= MiscHeader.WHITE) {
        MiscHeader.setColour(o, MiscHeader.GREY);
        MutatorContext owner = RVMThread.threadMonotonically[MiscHeader.getOriginalAllocatingThread(o)];
        if (owner != null) {
