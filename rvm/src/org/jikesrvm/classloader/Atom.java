...
public final class Atom {
  ...
  private static final byte[][] RVM_CLASS_ARRAY_PREFIXES =
      {"Lorg/jikesrvm/".getBytes(), "Lorg/vmmagic/".getBytes(), "Lorg/mmtk/".getBytes(), "[Lorg/jikesrvm/".getBytes(), "[Lorg/vmmagic/".getBytes(), "[Lorg/mmtk/".getBytes()};
  ...
  @Pure
  public boolean isRVMaDescriptor() {
    outer:
    for (final byte[] test : RVM_CLASS_ARRAY_PREFIXES) {
      if (test.length > val.length) continue;
      for (int j = 0; j < test.length; j++) {
        if (val[j] != test[j]) {
          continue outer;
