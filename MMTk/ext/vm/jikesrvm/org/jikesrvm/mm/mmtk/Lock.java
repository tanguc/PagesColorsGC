    ...
    if (!acquired) {
      for (;;) {
        ...
        if ((oldState==CLEAR &&
          ...
          Magic.sync();
          state=LOCKED_QUEUED;
          me.monitorNI().lockNoHandshakeNI();
          while (queue.isQueuedNI(me)) {
            // use await instead of waitNicely because this is NOT a GC point!
            me.monitorNI().waitNoHandshakeNI();
          }
          me.monitorNI().unlockNI();
        }
      }
    }
    thread = me;
    ...
    for (;;) {
      ...
      if (oldState==LOCKED &&
        ...
        break;
                 ...
                 Magic.attemptInt(this,offset,LOCKED_QUEUED,QUEUEING)) {
        RVMThread toAwaken=queue.dequeueNI();
        if (VM.VerifyAssertions) VM._assert(toAwaken!=null);
        boolean queueEmpty=queue.isEmptyNI();
        Magic.sync();
        if (queueEmpty) {
          state=CLEAR;
        } else {
        ...
        toAwaken.monitorNI().lockedBroadcastNoHandshakeNI();
        break;
      }
    }
  }
