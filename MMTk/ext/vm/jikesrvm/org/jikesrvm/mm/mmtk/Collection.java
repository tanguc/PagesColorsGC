...
public class Collection extends org.mmtk.vm.Collection {
  ...
  @Override
  @Interruptible
  public void spawnTICollectorContext(CollectorContext context) {
    byte[] stack = MemoryManager.newStack(ArchitectureSpecific.StackframeLayoutConstants.STACK_SIZE_COLLECTOR);
    CollectorThread t = new CollectorThread(stack, context, true);
    t.start();
  }
  ...
  @Override
  @Unpreemptible
  public void stopGlobaliserThread() {
    if (Plan.GLOBALISE_ON) {
      RVMThread.blockMutatorForGC(GlobaliserThread.threadID);
    }
  }
  ...
  @Override
  @Unpreemptible
  public void resumeGlobaliserThread() {
    if (Plan.GLOBALISE_ON) {
      RVMThread.unblockMutatorForGC(GlobaliserThread.threadID);
    }
  }
  ...
  @Unpreemptible
  @NoInstrument
  public void blockForTIGC() {
    RVMThread t=RVMThread.getCurrentThread();
    t.assertAcceptableStates(RVMThread.IN_JAVA, RVMThread.IN_JAVA_TO_BLOCK);
    t.isBlockingForTIGC = true;
    //GlobaliserThread.prepareForTIGC(t.getMonotonicId());
    RVMThread.getCurrentThread().block(RVMThread.TIGCBlockAdapter);
    //GlobaliserThread.exitTIGC(t.getMonotonicId());
    t.isBlockingForTIGC = false;
  }
  
  @Override
  @Unpreemptible
  public void waitUntilMutatorBlockedForTIGC(int monotonicID) {
    RVMThread.waitUntilMutatorBlockedForTIGC(monotonicID);
  }
  
  @Override
  @Unpreemptible
  public void resumeMutatorFromTIGC(int threadID) {
    RVMThread.unblockMutatorForTIGC(threadID);
  }
}
