 ...
 */
@Uninterruptible class SynchronisedLocalQueue extends SynchronisedLocalSSB {
   ...
   */
  SynchronisedLocalQueue(SharedDeque queue) {
    super(queue);
  }
 ...
 /****************************************************************************
  ...
  @Inline
  @NoInstrument
  protected final boolean checkDequeueNI(int arity) {
    if (bufferOffsetNI(head).isZero()) {
      return dequeueUnderflowNI(arity);
    } else {
      if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(bufferOffsetNI(head).sGE(Word.fromIntZeroExtend(arity).lsh(LOG_BYTES_IN_ADDRESS).toOffset()));
      return true;
    }
  }
  ...
  @Inline
  @NoInstrument
  protected final Address uncheckedDequeueNI(){
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(bufferOffsetNI(head).sGE(Offset.fromIntZeroExtend(BYTES_IN_ADDRESS)));
    head = head.minus(BYTES_IN_ADDRESS);
    return head.loadAddress();
  }
  ...
  @NoInstrument
  protected final boolean headStarvedNI(int arity) {
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(arity == queue.getArityNI());

    // If the tail has entries...
    if (tail.NE(tailBufferEnd)) {
      head = normalizeTailNI(arity).plus(BYTES_IN_ADDRESS);
      tail = Deque.TAIL_INITIAL_VALUE;
      tailBufferEnd = Deque.TAIL_INITIAL_VALUE;
      // Return that we acquired more entries
      return false;
    ...
    // Wait for another entry to materialize...
    lock.acquireNI();
    head = queue.dequeueAndWaitNI(arity);
    lock.releaseNI();
    // return true if a) there is a head buffer, and b) it is non-empty
    return (head.EQ(Deque.HEAD_INITIAL_VALUE) || bufferOffsetNI(head).isZero());
  }
  ...
  @NoInline
  @NoInstrument
  private boolean dequeueUnderflowNI(int arity) {
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(arity == queue.getArityNI());
    do {
      lock.acquireNI();
      if (head.NE(Deque.HEAD_INITIAL_VALUE))
        queue.free(head);
      head = queue.dequeueNI(arity);
      lock.releaseNI();
    } while (head.NE(Deque.HEAD_INITIAL_VALUE) && bufferOffsetNI(head).isZero());

    if (head.EQ(Deque.HEAD_INITIAL_VALUE))
      return !headStarvedNI(arity);
