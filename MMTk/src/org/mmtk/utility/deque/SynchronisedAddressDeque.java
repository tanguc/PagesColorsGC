...
/**
 * This supports <i>synchronized</i> enqueuing and dequeuing of addresses
 */
@Uninterruptible public class SynchronisedAddressDeque extends SynchronisedLocalDeque {
   ...
   */
  public SynchronisedAddressDeque(String n, SharedDeque queue) {
    super(queue);
    name = n;
  }
  ...
  @Inline
  @NoInstrument
  public final void insertNI(Address addr) {
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(!addr.isZero());
    checkTailInsertNI(1);
    uncheckedTailInsertNI(addr);
  }
  ...
  @Inline
  @NoInstrument
  public final Address popNI() {
    if (checkDequeueNI(1)) {
      return uncheckedDequeueNI();
    } else {
      return Address.zero();
    }
  }
  ...
  @Inline
  @NoInstrument
  public final boolean isEmptyNI() {
    return !checkDequeueNI(1);
  }
