 ...
 */
@Uninterruptible class SynchronisedLocalSSB extends Deque {
   ...
   */
  SynchronisedLocalSSB(SharedDeque queue) {
    lock = VM.newLock("");
    this.queue = queue;
    resetLocal();
  }
 ...
 /****************************************************************************
  ...
  @NoInstrument
  @Inline(value=Inline.When.AssertionsDisabled)
  protected final void checkTailInsertNI(int arity) {
    if (bufferOffsetNI(tail).isZero()) {
      tailOverflowNI(arity);
    }
    else if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(bufferOffsetNI(tail).sGE(Word.fromIntZeroExtend(arity).lsh(LOG_BYTES_IN_ADDRESS).toOffset()));
  }
  ...
  @NoInstrument
  @Inline(value=Inline.When.AssertionsDisabled)
  protected final void uncheckedTailInsertNI(Address value) {
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(bufferOffsetNI(tail).sGE(Offset.fromIntZeroExtend(BYTES_IN_ADDRESS)));
    tail = tail.minus(BYTES_IN_ADDRESS);
    tail.store(value);
    // if (Interface.VerifyAssertions) enqueued++;
  }
  ...
  @NoInstrument
  protected final Address normalizeTailNI(int arity) {
    Address src = tail;
    Address tgt = bufferFirstNI(tail);
    Address last = tgt.plus(bufferLastOffsetNI(arity).minus(bufferOffsetNI(tail)));
    while (tgt.LE(last)) {
      tgt.store(src.loadAddress());
      src = src.plus(BYTES_IN_ADDRESS);
      tgt = tgt.plus(BYTES_IN_ADDRESS);
  ...
  @NoInstrument
  @Inline
  protected final Offset bufferSentinelNI(int arity) {
    return bufferLastOffsetNI(arity).plus(BYTES_IN_ADDRESS);
  }
  ...
  @NoInstrument
  private void tailOverflowNI(int arity) {
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(arity == queue.getArityNI());
    if (tail.NE(Deque.TAIL_INITIAL_VALUE)) {
      closeAndEnqueueTailNI(arity);
    }
    lock.acquireNI();
    tail = queue.allocNI().plus(bufferSentinelNI(arity));
    tailBufferEnd = tail;
    lock.releaseNI();
  }
  ...
  @NoInline
  @NoInstrument
  private void closeAndEnqueueTailNI(int arity) {
    Address last;
    if (!bufferOffsetNI(tail).isZero()) {
      // prematurely closed
      last = normalizeTailNI(arity);
    } else {
      // a full tail buffer
      last = tailBufferEnd.minus(BYTES_IN_ADDRESS);
    }
    lock.acquireNI();
    queue.enqueueNI(last.plus(BYTES_IN_ADDRESS), arity, true);
    lock.releaseNI();
  }
