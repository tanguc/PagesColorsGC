...
public abstract class Plan {
  ...
  public static final boolean WRITE_BARRIERS_ON = true;
  public static final boolean READ_BARRIERS_ON = true;
  public static final boolean GLOBALISE_ON = true && READ_BARRIERS_ON && WRITE_BARRIERS_ON;
  public static final boolean REMSET_ON = true && READ_BARRIERS_ON && WRITE_BARRIERS_ON;
  public static final boolean TIGC_ON = true && REMSET_ON && READ_BARRIERS_ON && WRITE_BARRIERS_ON && GLOBALISE_ON;
  public static final boolean PERFORMANCE_RUN = true;
  public static final boolean SETBIND = true;
  
  public static final int MAX_TI = 300;
  
  //Static fields:
  private static int numPreTIThreads;
  public static boolean STOP_TIGC = false;
  public static volatile boolean harnessWarmupFinished = false;
  public static boolean reportedAllLiveObjects = false;
  public static boolean reportAllLiveObjects = false;
  
  public static final int DEFAULT_ADVICE = Plan.WHITE;
  ...
  public static void endTiming() {
    long endTime = VM.statistics.nanoTime();
    VM.assertions.logStr("Seconds Elapsed: ");
    VM.assertions.logLongln((long) ((endTime - startTime) / 1000000000));
    VM.assertions.logStr("Nanos Elapsed: ");
    VM.assertions.logLongln(endTime - startTime);
  }
  
  /* Object Colour values */
  /* DO NOT UPDATE THESE WITHOUT COPYING UPDATES TO MiscHeader.java */
  ...
  @Unpreemptible
  public static void handleLastCollectionRequest() {
  ...
  @Unpreemptible
  public static void lastGC() {
    if (Plan.reportedAllLiveObjects) {
      VM.assertions.fail("Already reported live objects");
    } 
  
    Plan.reportAllLiveObjects = true;
    Plan.handleLastCollectionRequest();
  }
...
//  public static final int LOS_SIZE_THRESHOLD = SegregatedFreeListSpace.MAX_CELL_SIZE;
 ...
 // public static final boolean REQUIRES_LOS = VM.activePlan.constraints().requiresLOS();
  ...
  public Plan() {
    ...
    Options.allocAdviceFile = new AllocAdviceFile();
    Options.pagesBetweenTIGC = new PagesBetweenTIGC();
    Options.reclamationThreshold = new ReclamationThreshold();
    Options.globaliseLevel = new GlobaliseLevel();
  ...
  public static void handleAllocationAdvice() {
  	// Read in advice file, if given.
  	if (Options.allocAdviceFile.getValue() != null) {
  	  if (Options.verbose.getValue() > 1) {
  	    Log.write("Reading allocation advice from: ");
  	    Log.writeln(Options.allocAdviceFile.getValue());
  	  }
  	  VM.activePlan.readAdviceFile(Options.allocAdviceFile.getValue());
  	  if (Options.verbose.getValue() > 1) {
  	    Log.writeln("Finished reading allocation advice.");
  	  }
  	}
  	else {
  	  Log.writeln("No advice file");
  	  //VM.assertions.fail("No advice file");
  	}
  }
  ...
  protected void spawnCollectorThreads(int numThreads) {
    ...
    numPreTIThreads = VM.collection.getThreads();
    VM.assertions.logStr("n Pre TI Threads: ");
    VM.assertions.logIntln(numPreTIThreads);
       
    Plan.threadIndependentWorkers = new ParallelCollector[MAX_TI];
    Phase.tiPhases = new ThreadIndependentPhase[Plan.MAX_TI];
    for (int i = 0; i < Plan.threadIndependentWorkers.length; i++) {
      try {
        Phase.tiPhases[i] = new ThreadIndependentPhase();
          
        threadIndependentWorkers[i] = defaultCollectorContext.newInstance(); //org.mmtk.plan.marksweep.MSCollector.class.newInstance();
        threadIndependentWorkers[i].group = null;
        threadIndependentWorkers[i].workerOrdinal = 0;
        threadIndependentWorkers[i].isThreadIndependent = true;
        threadIndependentWorkers[i].triggerCount = 1;
        threadIndependentWorkers[i].collectorSlot = i;
        threadIndependentWorkers[i].lock = VM.newHeavyCondLock("TILock");
        VM.collection.spawnTICollectorContext(threadIndependentWorkers[i]);
    
      } catch (Throwable t) {
        VM.assertions.fail("Error creating collector context '" + defaultCollectorContext.getName() + "': " + t.toString());
      }
    }
  ...
  public static ParallelCollector[] threadIndependentWorkers = new ParallelCollector[MAX_TI];
  public static int translateMutatorIDToIndex(int mut) {
    return (mut <= numPreTIThreads ? mut - 1 : mut - Plan.MAX_TI - 1); 
  }
