...
public final class MarkSweepSpace extends SegregatedFreeListSpace {
  ...
  public void prepare(boolean gcWholeMS) {
    reclaimedCellsSize = 0;
    reclaimedBlocksSize = 0;
    ...
    if (HEADER_MARK_BITS) {
      if (testAndMark(object)) {
        ...
        if (!Plan.PERFORMANCE_RUN) {
          /*switch (VM.objectModel.getColour(object)) {
          case Plan.BLACK: trace.blackObjects++; break;
          case Plan.GREY: trace.greyObjects++; break;
          case Plan.WHITE: trace.whiteObjects++; break;
          default: break;
          }*/
          if (VM.objectModel.getColour(object) == Plan.WHITE) trace.whiteObjects++;
        }
      ...
      if (testAndMark(object)) {
        if (addToRemset) {
          ...
          if (Plan.REMSET_ON) {
            ...
            if (trace.collectorRemsetBuffers[ownerId] != null) {
              trace.collectorRemsetBuffers[ownerId].insertNI(object.toAddress());
            }
            else {
              VM.assertions.logStrln("[Warning]: tracing object with no thread context");
            }
