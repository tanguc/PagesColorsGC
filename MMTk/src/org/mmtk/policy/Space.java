...
public abstract class Space {
  ...
  public static final int MAX_SPACES = 2000; // quite arbitrary
  ...
  public boolean isThreadLocalSpace = false;
  public int threadLocalIndex = -1;
  
  public long reclaimedCellsSize = 0;
  public long reclaimedBlocksSize = 0;
  ...
  @NoInstrument
  public final int getIndexNI() { return index; }
  ...
  @Inline
  public static boolean isMappedAddressNI(Address address) {
    return Map.getSpaceForAddressNI(address) != null && Mmapper.addressIsMappedNI(address);
  }
  ...
  @Inline
  public static Space getSpaceForObjectNI(ObjectReference object) {
    return Map.getSpaceForAddressNI(VM.objectModel.refToAddress(object));
  }
  ...
  public void tigc(int pagesReserved) {
    int mutID = VM.activePlan.getThreadID();
    int index = Plan.translateMutatorIDToIndex(mutID);
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(Plan.threadIndependentWorkers != null);
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(index < Plan.threadIndependentWorkers.length);
    if (VM.VERIFY_ASSERTIONS) VM.assertions._assert(Plan.threadIndependentWorkers[index] != null);
    Plan.threadIndependentWorkers[index].setMutator(mutID);
    Plan.threadIndependentWorkers[index].requestTIGC();
    pr.clearRequest(pagesReserved);
    VM.collection.blockForTIGC();
  }
  
  //static int monotonicPagesAcquired = 0;
  private int monotonicPagesAcquired = 0;
  //private Lock mPAlock = VM.newLock("mPA");
  private int prevMonotonicPagesAcquired = 0;
  public int kbFreedLastTIGC = Integer.MAX_VALUE;
  
  //static int threadsInTIGC = 0;
  //private Lock counterLock = VM.newLock("tigcCounterLock");
  /**
  ...
  public final Address acquire(int pages) {
    ...
    if (allowPoll && VM.activePlan.global().poll(Map.getAvailableDiscontiguousChunks() < 3, this)) {
      pr.clearRequest(pagesReserved);
      VM.collection.blockForGC();
      return Address.zero(); // GC required, return failure
    }
    else if (Map.getAvailableDiscontiguousChunks() > 3 && !Plan.STOP_TIGC && Plan.TIGC_ON && this.isThreadLocalSpace && VM.activePlan.fullyBooted() && VM.activePlan.isBenchmarkThread(VM.activePlan.getThreadID()) && Stats.gcCount() >= 1) {
    	boolean meetsElapsedPageRequirement = (prevMonotonicPagesAcquired + Options.pagesBetweenTIGC.getValue()) <= monotonicPagesAcquired;
    	boolean meetsPagesFreedRequirement = kbFreedLastTIGC >= Options.reclamationThreshold.getValue();
    	
    	/*VM.assertions.logStr("Request ");
    	VM.assertions.logInt(prevMonotonicPagesAcquired);
    	VM.assertions.logStr(" ");
    	VM.assertions.logInt(Options.pagesBetweenTIGC.getValue());
    	VM.assertions.logStr(" ");
    	VM.assertions.logInt(monotonicPagesAcquired);
    	VM.assertions.logStr(" ");
    	VM.assertions.logInt(kbFreedLastTIGC);
    	VM.assertions.logStr(" ");
    	VM.assertions.logInt(Options.reclamationThreshold.getValue());
    	VM.assertions.logStr(" ");
    	VM.assertions.logIntln(meetsElapsedPageRequirement && meetsPagesFreedRequirement ? 1010101 : 101);*/
    	
      if (meetsElapsedPageRequirement && meetsPagesFreedRequirement) {
        prevMonotonicPagesAcquired = monotonicPagesAcquired;
        /*counterLock.acquireNI();
        threadsInTIGC++;
        counterLock.releaseNI();*/
        tigc(pagesReserved);
        /*counterLock.acquireNI();
        threadsInTIGC--;
        counterLock.releaseNI();*/
    ...
    //mPAlock.acquireNI();
    monotonicPagesAcquired += pagesReserved;
    //mPAlock.releaseNI();
   ...
   */
  public static int getPagesReserved() {
    int pages = 0;
  ...
  public ObjectReference traceObject(TransitiveClosure trace, ObjectReference object, boolean addToRemset) {
    return traceObject(trace, object);
  }
